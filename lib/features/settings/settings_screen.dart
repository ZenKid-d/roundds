...
/// Обход блокировок для России: тумблер + свои зеркала Piped/Invidious.
class _RfBypass extends ConsumerStatefulWidget {
  const _RfBypass();
  @override
  ConsumerState<_RfBypass> createState() => _RfBypassState();
}

class _RfBypassState extends ConsumerState<_RfBypass> {
  final _c = TextEditingController();

  List<String> get _instances =>
      ref.read(prefsProvider).getStringList('rf_instances') ?? const [];
  bool get _enabled => ref.read(prefsProvider).getBool('rf_bypass') ?? false;

  void _apply() {
    final prefs = ref.read(prefsProvider);
    ref.read(youtubeSourceProvider).configureBypass(
  ...
  Future<void> _setEnabled(bool v) async {
    await ref.read(prefsProvider).setBool('rf_bypass', v);
    _apply();
    setState(() {});
  }

  Future<void> _add() async {
    var url = _c.text.trim();
    if (url.isEmpty) return;
    if (!url.startsWith('http')) url = 'https://$url';
    url = url.replaceAll(RegExp(r'/+$'), '');
    final list = [..._instances];
    if (!list.contains(url)) list.insert(0, url);
    await ref.read(prefsProvider).setStringList('rf_instances', list);
    _c.clear();
    _apply();
    setState(() {});
  }
  ...
  Future<void> _remove(String url) async {
    final list = _instances.where((e) => e != url).toList();
    await ref.read(prefsProvider).setStringList('rf_instances', list);
    _apply();
    setState(() {});
  }
    ...
    return Column(
      ...
      children: [
        SwitchListTile(
          contentPadding: EdgeInsets.zero,
          value: enabled,
          title: const Text('Обход блокировок для YouTube'),
          subtitle: Text(
              'Поток YouTube идёт через зеркало (Piped/Invidious) — играет '
              'без VPN, если зеркало доступно из РФ.',
              style: TextStyle(color: AppColors.white45, fontSize: 11)),
        ...
        if (enabled) ...[
          ...
          Text(
            'Своё зеркало (свой сервер) — самый стабильный вариант: публичные '
            'зеркала бывают перегружены. Укажите адрес вашего Piped API или '
            'Invidious. Свои зеркала пробуются первыми.',
            style: TextStyle(color: AppColors.white45, fontSize: 11.5),
          ),
          const SizedBox(height: 8),
          Row(
            children: [
              ...
              IconButton(
                  icon: const Icon(Icons.add_circle), onPressed: _add),
            ],
          ),
          for (final u in instances)
            ListTile(
              dense: true,
              contentPadding: EdgeInsets.zero,
              leading: const Icon(Icons.dns_outlined, size: 20),
              title: Text(u, style: const TextStyle(fontSize: 12.5)),
              trailing: IconButton(
                icon: Icon(Icons.close, size: 18, color: AppColors.white60),
                onPressed: () => _remove(u),
              ),
            ),
          if (instances.isEmpty)
            Padding(
              ...
              child: Text(
                'Свои зеркала не добавлены — используются встроенные публичные '
